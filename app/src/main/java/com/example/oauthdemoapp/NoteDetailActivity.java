...
public class NoteDetailActivity extends AppCompatActivity {
    ...
    private void saveNote() {
        ...
        authManager.performActionWithFreshTokens(new AuthManager.AuthActionCallback() {
            ...
            public void onTokenAvailable(String accessToken) {
                ...
                if (isEditMode) {
                    call = apiService.updateNote("Bearer " + accessToken, noteId, requestBody);
                } else {
                    call = apiService.createNote("Bearer " + accessToken, requestBody);
                }
            ...
            public void onError(String errorMessage) {
                showLoading(false);
                Toast.makeText(NoteDetailActivity.this,
                        "Error de autenticación: " + errorMessage, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void deleteNote() {
        ...
        authManager.performActionWithFreshTokens(new AuthManager.AuthActionCallback() {
            @Override
            public void onTokenAvailable(String accessToken) {
                Call<ResponseBody> call = apiService.deleteNote("Bearer " + accessToken, noteId);
                call.enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        showLoading(false);
